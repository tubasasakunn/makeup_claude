...
def main():
    ...
    for img_path in sorted(IMGS_DIR.glob("*.png")):
        ...
        if image is None or fm.detect(image) is None:
            continue
        r = analyze(fm)
        print(f"  lip={r.lip_ratio:.2f}  phil={r.philtrum_ratio:.2f}  "
              f"mouth/face={r.mouth_to_face_ratio*100:.1f}%  [{r.overall}]")
    ...
    if results:
        ...
        for stem, res in results:
            ...
            cv2.putText(
                lab,
                f"{ROMAJI.get(stem,stem)}  lip={res.lip_ratio:.2f}  "
                f"phil={res.philtrum_ratio:.2f}  [{res.overall}]",
