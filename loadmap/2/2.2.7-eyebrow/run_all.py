...
def main():
    ...
    for img_path in sorted(IMGS_DIR.glob("*.png")):
        ...
        if image is None or fm.detect(image) is None:
            continue
        r = analyze(fm)
        print(f"  sym={r.symmetry_score*100:.1f}%  [{r.category}]")
        print(f"  R angle={r.right.angle_head_to_peak_deg:.1f} ratio={r.right.peak_ratio:.2f}")
    ...
    if results:
        ...
        for stem, res in results:
            ...
            cv2.putText(
                lab,
                f"{ROMAJI.get(stem,stem)}  sym={res.symmetry_score*100:.0f}%  [{res.category}]",
