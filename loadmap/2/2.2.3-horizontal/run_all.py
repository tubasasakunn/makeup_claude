...
ROMAJI = {"ベース": "base", "丸顔": "marugao", "卵": "tamago",
          "逆三角": "gyakusankaku", "面長": "omonaga"}
...
def main():
    ...
    for img_path in sorted(IMGS_DIR.glob("*.png")):
        ...
        if image is None or fm.detect(image) is None:
            continue
        r = analyze(fm)
        print(f"  norm: " + " : ".join(f"{v:.2f}" for v in r.seg_norm))
        print(f"  gap/eye={r.eye_gap_ratio:.3f}  category={r.category}")
    ...
    if results:
        ...
        for stem, res in results:
            ...
            if p is None:
                continue
            lab = np.zeros((36, p.shape[1], 3), dtype=np.uint8)
            lab[:] = (40, 40, 40)
            cv2.putText(
                lab,
                f"{ROMAJI.get(stem, stem)}  gap/eye={res.eye_gap_ratio:.2f}  [{res.category}]",
        ...
        cv2.imwrite(str(OUTPUT_DIR / "_summary.png"), grid)
        print("\nサマリー: _summary.png")
    print("\n完了")
