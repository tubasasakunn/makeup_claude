...
## M2 (完了) — 処方適用パイプライン
base → highlight → shadow → eye(5 areas) → eyebrow の順で Phase 1 各関数を呼び出す。
`apply_prescription(image, fm, rx)` で 1 枚の処方を完全合成。

## M3 (完了) — ルール R2-R5 を追加

| ルール | 入力 | 出力 |
|---|---|---|
| R2 | skeletal.type | highlight.areas (base-* / marugao-* / omonaga-*) |
| R3 | skeletal.type | shadow.enabled + areas (小顔効果) |
| R4 | eye.ideal_ratio_loss (2.2.4) | eyeshadow_base intensity 底上げ (+0.00 〜 +0.20) |
| R5 | golden_score (2.2.8)         | 全体 intensity スケール (×0.85 / ×1.00 / ×1.15) |

エリアセット:
- `BASE_HL`    = base_t-zone / c-zone / under-eye / megasira / zintyuu
- `MARU_HL`    = marugao_t-zone / c-zone / ago (縦方向)
- `OMONAGA_HL` = omonaga_t-zone / c-zone (狭め)

R3 シャドウ:
- round, base → `marugao-side` (エラ/頬を削る)
- long        → `omonaga-upper, omonaga-lower` (縦を短く)
- oval, inverted_triangle → OFF
...
## 次 (M4)
- 3.3 E2E + レポート可視化 (判定 → 処方 → 仕上がり を 1 枚のレポートに)
- ルールの色/強度パラメータ化の更なる拡張
